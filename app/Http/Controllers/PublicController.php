...
use App\Mail\PackageEnquiryMail;
use App\Mail\PackageEnquiryConfirmationMail;
...
class PublicController extends Controller
{
    ...
    {
        $blogs = Blog::where('is_published', true)->latest()->paginate(7);
    ...
    public function storePackageEnquiry(Request $request, Package $package)
    {
        ...
        try {
            ...
            // 1. Notify admin
            Mail::to($adminEmail)->send(new PackageEnquiryMail($enquiry, $package));

            // 2. Send confirmation to customer
            Mail::to($validated['email'])->send(new PackageEnquiryConfirmationMail($enquiry, $package));

        } catch (\Throwable $e) {
            Log::error('PackageEnquiry mail failed: ' . $e->getMessage());
        }

        return redirect()->back()->with(
            'success',
            'Your booking enquiry for ' . $package->title . ' has been submitted successfully! Our tour expert will contact you soon.'
        );
    }

    public function cabs(Request $request)
    {
        $cabs = \App\Models\CabBookingPackage::where('status', true)->latest()->get();
    ...
    public function boatRides()
    {
        $service = Service::where('title', 'LIKE', '%Boat Ride%')->first();
        $packages = collect();
        if ($service) {
            $packages = Package::where('service_id', $service->id)->where('status', true)->latest()->get();
        }
        $page = Page::where('slug', 'boat-rides')->first() ?? Page::create(['page_name' => 'Boat Rides', 'slug' => 'boat-rides', 'status' => true]);
        $sections = $page->sections()->where('status', true)->get()->keyBy('section_name');
