        ...
        <form action="{{ route('admin.packages.update', $package) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="slug" class="form-control @error('slug') is-invalid @enderror" value="{{ old('slug', $package->slug) }}" placeholder="e.g. best-varanasi-tour">
                    @error('slug')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    <small class="text-muted">Will remain unchanged or auto-generated if left blank.</small>
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-4">
                    <label class="form-label fw-bold">Cab Booking Mapping</label>
                    <select name="cab_booking_package_id" class="form-select @error('cab_booking_package_id') is-invalid @enderror">
                        <option value="">Select Cab Package (Optional)</option>
                        @foreach ($cabPackages as $cab)
                        <option value="{{ $cab->id }}" {{ old('cab_booking_package_id', $package->cab_booking_package_id) == $cab->id ? 'selected' : '' }}>{{ $cab->cab_name }} ({{ $cab->vehicle_type }})</option>
                        @endforeach
                    </select>
                    @error('cab_booking_package_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="col-md-4">
                    <label class="form-label fw-bold">Start Location</label>
                    <input type="text" name="start_location" class="form-control" value="{{ old('start_location', $package->start_location) }}" placeholder="e.g. Kashi / Varanasi">
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea name="itinerary" rows="8" class="form-control" placeholder="Day 1 | Arrival in Varanasi | Rest and acclimatize.&#10;Day 2 | Varanasi to Sham Valley | Explore Sham Valley.">{{ old('itinerary', trim($itineraryText)) }}</textarea>
                ...
                <div class="col-12">
                    ...
                    <input type="text" name="meta_title" class="form-control @error('meta_title') is-invalid @enderror" value="{{ old('meta_title', $package->meta_title) }}" placeholder="e.g. Best Bike Tour in Kashi | Kashi Tourism">
                ...
                <div class="col-12">
                    ...
                    <textarea name="meta_keywords" rows="2" class="form-control @error('meta_keywords') is-invalid @enderror" placeholder="e.g. bike tour Kashi, Varanasi tour, motorbiking Kashi">{{ old('meta_keywords', $package->meta_keywords) }}</textarea>
                    @error('meta_keywords')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="col-12">
                    <label class="form-label fw-bold">Custom Meta Tags (HTML)</label>
                    <textarea name="meta_tags" class="form-control" rows="4" placeholder="Enter custom HTML meta tags (e.g. Open Graph, Canonical, JSON-LD)">{{ old('meta_tags', $package->meta_tags) }}</textarea>
                </div>
                        ...
                        <div class="col-md-3 col-sm-6 image-container">
                            <div class="card h-100 border">
                                ...
                                <div class="card-body p-2">
                                    ...
                                    <input type="text" name="alt_text[{{ $img }}]" class="form-control form-control-sm" value="{{ $package->alt_text[$img] ?? '' }}" placeholder="Image description">
                                </div>
                            </div>
                        </div>
                        @endforeach
