        ...
        <div class="card-body p-4 p-lg-5">
            <form action="{{ route('admin.galleries.update', $gallery) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-4">
                    <div class="col-md-8">
                        <div class="mb-4">
                            <label class="form-label fw-bold small text-muted text-uppercase mb-2">Image Title
                                (Optional)</label>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="category"
                                ...
                                @foreach(['Architecture', 'Interiors', 'Life & Style', 'Dining', 'Wellness'] as $cat)
                                    <option value="{{ $cat }}" {{ $gallery->category == $cat ? 'selected' : '' }}>{{ $cat }}
                                    </option>
                                @endforeach
                            </select>
                            @error('category')
                    ...
                    <div class="col-md-4">
                        <div class="mb-4">
                            ...
                            @if($gallery->image)
                                <div class="mt-3">
                                    ...
                                    <img src="{{ asset('storage/' . $gallery->image) }}" class="img-fluid rounded-3 border"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="alt_text"
                                class="form-control form-control-lg @error('alt_text') is-invalid @enderror"
                                value="{{ old('alt_text', $gallery->alt_text) }}" placeholder="Alt text description">
                            @error('alt_text')
                            <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                ...
                <div class="mt-4 pt-4 border-top">
                    <button type="submit" class="btn btn-navy px-5 py-3 fw-bold rounded-pill shadow-premium">UPDATE GALLERY
                        ITEM</button>
                    <a href="{{ route('admin.galleries.index') }}"
                        class="btn btn-link text-muted fw-bold ms-3 text-decoration-none">Cancel</a>
                </div>
            </form>
        </div>
    </div>
