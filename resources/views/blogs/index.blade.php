...
@php
    ...
    $gridBlogs    = $allBlogs->skip(1)->values();
@endphp
...
<div class="vk-blog-body">
    <div class="container">
        ...
        @if($featuredBlog)
        <a href="{{ route('blogs.show', $featuredBlog->slug) }}" class="vk-feat">
            <div class="vk-feat-img">
                <span class="vk-feat-badge">✦ Featured</span>
                <img src="{{ $featuredBlog->featured_image ? asset('storage/'.$featuredBlog->featured_image) : 'https://images.unsplash.com/photo-1598897516650-df69c2fdd6a7?auto=format&fit=crop&q=80&w=900' }}"
                     alt="{{ $featuredBlog->title }}">
            </div>
            <div class="vk-feat-body">
                <div class="vk-meta">
                    <span><i class="bi bi-calendar3"></i> {{ $featuredBlog->created_at->format('d M Y') }}</span>
                    <span><i class="bi bi-clock"></i> {{ $featuredBlog->reading_time ?? max(1, ceil(str_word_count(strip_tags($featuredBlog->content)) / 200)) }} min read</span>
                    <span><i class="bi bi-person"></i> {{ $featuredBlog->user?->name ?? 'Visit Kashi' }}</span>
                </div>
                <h2 class="vk-feat-title">{{ $featuredBlog->title }}</h2>
                <p class="vk-feat-excerpt">{{ $featuredBlog->short_description ?: Str::limit(strip_tags($featuredBlog->content), 200) }}</p>
                <span class="vk-read-btn">Read Full Article <i class="bi bi-arrow-right"></i></span>
            </div>
        </a>
        @endif
